...
class Regen_access_token_for_switching_organization(IRegen_access_token_for_switching_organization):
    ...
    async def aexecute(self, request: "Request") -> "Response":
        ...
        token_data: TokenData = self._user_context

        refresh_token = self._refresh_token_collection.find_one({"_id": request.refresh_token_id})
        ...
        ## [?] if token_data.org_id == request.org_id -> no change
        # if token_data.organization_id == request_organization_id:
        #     return self.Response(
        #         access_token=self._jwt_service.encode_jwt_token(token_data), refresh_token_id=request.refresh_token_id
        #     )

        get_organization = await self._get_organization.aexecute(
            GetOrganizationByIdDep.Request(id=request_organization_id)
        )
        ...
        # revoke out-date refresh token
        update_data = {"revoked_at": get_utc_now()}
        updated_refresh_token = RefreshTokenModel(**refresh_token).model_copy(update=update_data)
